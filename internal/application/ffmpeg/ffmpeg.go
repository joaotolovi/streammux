...
func (m *Muxer) StartPlaceholderSession(ctx context.Context, introPath, loopPath, outputDir string) (*Session, error) {
	...
	// Build the input list and the concat filter. The intro (if any) is input 0
	// and plays once; the loop video (if any) is the last input and loops.
	args := []string{
		"-nostdin",
		"-hide_banner",
		"-nostats",
	...
	if loopPath != "" {
		args = append(args, "-stream_loop", "-1", "-i", loopPath)
